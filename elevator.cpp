...
try {
    ...
    if(maxFloors<5) maxFloors = 5;
    if(noOfElevators>10) noOfElevators = 10;
    if(maxPeople>8) maxPeople = 8;
    if(maxSpawnRate>10) maxSpawnRate = 30;
}
    ...
    details << endl << "The details of Passenger Spawns with ticks are given below: " << endl;
    details.close();
        printPMap(pMap);
    wtc.startSimulation();
    }
